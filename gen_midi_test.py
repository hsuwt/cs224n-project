...
chords_LM_onehot = np.genfromtxt('pred/pred_LM_onehot.csv', delimiter = ',')
...
for i in range(10):
    ...
    song_LM_onehot = util.Matrices_to_MIDI(mel[128*i:128*(i+1)],chords_LM_onehot[128*i:128*(i+1)])
    song_pair = util.Matrices_to_MIDI(mel[128*i:128*(i+1)],chords_pair[128*i:128*(i+1)])
    song_pair_L1diff = util.Matrices_to_MIDI(mel[128*i:128*(i+1)],chords_pair_L1diff[128*i:128*(i+1)])

    song_ground.write(midi_output_path+str(i)+'_ground.mid')
    song_LM.write(midi_output_path+str(i)+'_LM.mid')
    song_LM_onehot.write(midi_output_path+str(i)+'_LM_onehot.mid')
